...
async def get_public_supplier_directory(
    ...
    for unit in units:
        ...
        if q:
            q_l = _norm(q)
            if q_l not in _norm(group.nom if group else "") and q_l not in _norm(unit.supplier_code):
                continue
        ...
        for r in approved:
            ...
            plants.append(
                PublicPlantEntry(
                    ...
                    annual_spend_currency=r.annual_spend_currency,
                    lta=pld.lta if pld else None,
                    quality_certification=pld.quality_certification if pld else None,
        ...
        items.append(
            PublicSupplierEntry(
                ...
                product_type=getattr(unit, "product_type", None),
                product_category=getattr(unit, "product_category", None),
