...
class SupplierRepository:
    ...
    async def find_group_by_name(self, name: str) -> Optional[SupplierGroup]:
        ...
        stmt = select(SupplierGroup).where(
            SupplierGroup.nom == name, SupplierGroup.is_deleted.is_(False)
        )
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def find_unit_by_id(self, unit_id: int) -> Optional[SupplierUnit]:
        ...
        stmt = select(SupplierUnit).where(
            SupplierUnit.id_supplier_unit == unit_id, SupplierUnit.is_deleted.is_(False)
        )
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()

    async def find_unit_by_code(
        self, code: str, group_id: Optional[int] = None
    ) -> Optional[SupplierUnit]:
        """Find supplier unit by supplier code, optionally scoped to one group."""
        stmt = select(SupplierUnit).where(
            SupplierUnit.supplier_name == code, SupplierUnit.is_deleted.is_(False)
        )
        if group_id is None:
    ...
    async def find_certifications_by_unit(
        self, unit_id: int
    ...
    async def find_certification_by_id(
        self, cert_id: int
    ) -> Optional[SupplierCertification]:
